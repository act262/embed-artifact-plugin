...
public class InvokeManifestMerger extends AndroidVariantTask implements Supplier<File> {
    ...
    protected void doFullTaskAction() throws ManifestMerger2.MergeFailureException, IOException {
        ...
        // different here, replace with 'utf-8' charset
        OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(getOutputFile()), StandardCharsets.UTF_8);
        writer.write(mergingReport.getMergedDocument(MergingReport.MergedManifestKind.MERGED));
        writer.flush();
        writer.close();
    }
